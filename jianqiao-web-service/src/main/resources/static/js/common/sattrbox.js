...
$(function () {
    ...
    $('.MainCont').on('click', 'img.del_inputbox_all_sp', function () {
        ...
        if ($(this).parent('.input-box').parent('.inputbox_big').parent('.mainFrom').parent().parent('.ph_classify').find('.del_inputbox_all_sp').length <= 1) {
            ...
            $(this).parent('.input-box').parent('.inputbox_big').parent('.mainFrom').parent().remove();
            var len = $('.ph_classify > div').length;
            console.log(len)
            if(len <= 1){
                $('.MainCont input.add_specifications').removeAttr('disabled');
                $('.MainCont input.add_specifications').css({'background':'#10c8ef'})
            }
        }
    });
    //添加规格项
    ...
    function table_add(num,countNum) {
        ...
        for (var k = 0; k < tmpc.length; k++) {
            ...
            str +=
                '<tr>' +
                '<td  class="td_input">' + tmpc[k] + '<input type="hidden" ref="skus"   name="content" value="' + tmpc[k] + '" /></td>' +
                '<td class="td_input"><input type="text" class="table_input validate[required,custom[price], min[0]]" ref="skus"   name="price" id="'+priceID+'" value="" /></td>' +
                '<td class="td_input"><input type="text" class="table_input validate[required,custom[price], min[0]]" ref="skus"  name="salePrice"  id="'+countID+'"   onblur="setPurchasePrice('+k+',\'' +countID+'\');"  value="" /></td>' +
                '<td class="td_input"><input type="text" class="table_input validate[required,custom[price], min[0]]" ref="skus"  name="platfromIncome" id="'+platfID+'" readonly="readonly" onblur="setPlatfromIncome('+k+',\'' +platfID+'\');" value="" /></td>' +
                '<td class="td_input"><input type="text" class="table_input validate[required,custom[price], min[0]]" ref="skus"  name="hunterIncome" id="'+hunteID+'" readonly="readonly" value="" /></td>' +
                '<td class="td_input"><input type="text" class="table_input validate[required,custom[price], min[0]]" ref="skus" name="storeIncome" id="'+storeID+'" readonly="readonly" value="" /></td>' +
                /*'<td class="td_input"><input type="text" class="table_input validate[required,custom[price], min[0]]" ref="skus" name="buyingPrice" id="'+buyinID+'" readonly="readonly" value="" /></td>' +*/
                '<td class="td_input"><input type="text" class="table_input validate[required,custom[number],maxSize[15]]" ref="skus"  name="barCode" id="'+barcaID+'" onblur="queryBarcode(\''+barcaID+'\');" value="" /></td>' +
                ...
                '<td class="td_input"><input type="text" class="table_input validate[required, custom[integer], min[1],max[2147483647]]" ref="skus" name="num" value=""  onblur="compute();" /></td>' +
                '</tr>';
