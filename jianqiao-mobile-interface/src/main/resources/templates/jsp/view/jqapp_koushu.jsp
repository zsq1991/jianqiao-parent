...
<div id="banner">
	...
	<c:if test="${not empty consultation.address }">
						<img class="touxiang" src="${ctx_ali}/${consultation.address }">
		...
		<div class="inter_header clearfix">
			<div class="reporter_box lf">
				<div class="head lf">
					...
					</c:if><c:if test="${not empty consultationTop.address }">
			...
			<%-- 	<li class="inter_section_list">
				...
				<c:forEach items="${ftDetailList}" var="ftdl"  >
					<c:if test="${not empty ftdl.detailContent }">
						<%-- <img class="touxiang" src="${ctx_ali}/${consultationTop.address }"> --%>
					<pre class="huida">${ftdl.detailContent }</pre>
					</c:if>
					<c:if test="${not empty ftdl.address }">
						...
						<c:if test="${ftdl.type == 3}">
							<video preload="load" controls="controls" width="100%" height="500px">
...
<!-- 	<div class="view_item clearfix"  onclick="downloadApp('moreInfo')">
	...
	<div class="view_item check_all" style="border-bottom:0"  onclick="downloadApp('moreInfo')">
		<h4>查看全部</h4>
	</div>
</div>
<div class="comment">
	<h3 class="title">用户评论（<span>${consultationTop.commentNum }</span>条）</h3>
	<ul>
