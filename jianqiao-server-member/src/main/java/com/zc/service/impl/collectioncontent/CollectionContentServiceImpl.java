...
public class CollectionContentServiceImpl implements CollectionContentService {
    ...
    public Result mycollection(Member member, Integer page, Integer rows) {
        ...
        for (Map<String, Object> consultationId : myCollentent) {
            Map<String, Object> consulationDetail = consultationMapper
                    .getConsultationById(consultationId.get("consultation_id").toString());
        ...
        for (String consultation_id : set) {
            // String consultation_id = map.get("consultation_id");
            ...
            // 处理时间格式
            String chidAlreadyTime1 = DateUtils.dateFormat((Date) consulationDetail.get("createdTime"),
                    "yyyy/MM/dd HH:mm:ss");
            ...
            if (type.equals("0") || type.equals("2")) {// 0是访谈主题 1访谈内容 2口述主题
                ...
                consultationChidList = consultationMapper
                        .findConsultationChidById(Long.valueOf(consulationDetail.get("id").toString()));// 根据访谈或口述的id查询其子类
                // 取详情内容
                ...
                if (consultationChidList.size() > 0) {
                    for (Map<String, Object> consultationChidInfo : consultationChidList) {
                        // 处理时间格式
                        String chidAlreadyTime = DateUtils.dateFormat((Date) consultationChidInfo.get("createdTime"),
                                "yyyy/MM/dd HH:mm:ss");
                        ...
                        // 处理咨询图片
                        List<Map<String, Object>> consultationChidAttachmentList = consultationAttachmentService
                                .findConsultationAttachmentByConsultationId(
                                        Long.valueOf(consultationChidInfo.get("id").toString()));
                        consultationChidInfo.put("address", consultationChidAttachmentList);

                        // 处理访谈和口述专题的详情内容
                        Map<String, Object> map1 = consultationAttachmentService.findDetailContentByConsultationId(
                                Long.valueOf(consulationDetail.get("id").toString()));
            ...
            // 处理咨询图片
            List<Map<String, Object>> consultationAttachmentList = consultationAttachmentService
                    .findConsultationAttachmentByConsultationId(Long.valueOf(consulationDetail.get("id").toString()));
            consulationDetail.put("address", consultationAttachmentList);

            // 取详情内容
            Object detailContent = "";
            if (type.equals("4") || type.equals("6")) {
                Map<String, Object> map2 = consultationAttachmentService
                        .findDetailContentByConsultationId(Long.valueOf(consulationDetail.get("id").toString()));
